...
RDLATEST=$(curl https://api.github.com/repos/bryangerlach/rustdesk-server/releases/latest -s | grep "tag_name"| awk '{print substr($2, 2, length($2)-3) }')

echo "Installing Rustdesk Server"
if [ "${ARCH}" = "x86_64" ] ; then
wget "https://github.com/bryangerlach/rustdesk-server/releases/download/${RDLATEST}/rustdesk-server-linux-amd64.zip"
unzip rustdesk-server-linux-amd64.zip
mv amd64/* /opt/rustdesk/
elif [ "${ARCH}" = "armv7l" ] ; then
wget "https://github.com/bryangerlach/rustdesk-server/releases/download/${RDLATEST}/rustdesk-server-linux-armv7.zip"
unzip rustdesk-server-linux-armv7.zip
mv armv7/* /opt/rustdesk/
elif [ "${ARCH}" = "aarch64" ] ; then
wget "https://github.com/bryangerlach/rustdesk-server/releases/download/${RDLATEST}/rustdesk-server-linux-arm64v8.zip"
...
# Setup Systemd to launch webs
rustdeskwebs="$(cat << EOF
[Unit]
Description=Rustdesk Web Server
[Service]
Type=simple
LimitNOFILE=1000000
ExecStart=/opt/rustdesk/webs
WorkingDirectory=/opt/rustdesk/
User=${uname}
Group=${uname}
Restart=always
StandardOutput=append:/var/log/rustdesk/webserver.log
StandardError=append:/var/log/rustdesk/webserver.error
# Restart service after 10 seconds if node service crashes
RestartSec=10
[Install]
WantedBy=multi-user.target
...
function setuphttp () {
    ...
    echo -e "You can access your install scripts for clients by going to http://${wanip}:21114/install"
    echo -e "Username is admin and password is admin"
    echo -e "Please change admin password on first login"
    if [[ -z "$http" ]]; then
